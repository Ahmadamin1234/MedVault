...
export default function Sidebar({ isCollapsed, setIsCollapsed, allowedPages }) {
 ...
 const menuItems = [
    { name: "Dashboard", path: "/dashboard", key: "dashboard", icon: LayoutDashboard },
    { name: "Inventory", path: "/inventory", key: "inventory", icon: Package },
    { name: "Purchase Orders", path: "/purchase-orders", key: "purchase-orders", icon: FileText }, // 🍏 FIXED MATCH
    { name: "Suppliers", path: "/suppliers", key: "suppliers", icon: Truck },
    { name: "Expiry Alerts", path: "/expiry-alerts", key: "expiry-alerts", icon: AlertTriangle }, // 🍏 FIXED MATCH
    { name: "Sales & Billing", path: "/sales-billing", key: "sales-billing", icon: DollarSign }, // 🍏 FIXED MATCH
 ...
 const handleLogoutAction = async () => {
    if (!window.confirm("Are you sure you want to end your active session and sign out of MedVault?")) return;
    try {
      await logout(); // Calls Django POST /auth/logout/ to clear HTTP-Only cookies
      navigate("/login"); // Smoothly pushes user back to the sign-in canvas tree terminal
    } catch (err) {
      console.error("Failed to terminate security footprint session:", err);
      // Fallback routing enforcement if network connection is dropping
      localStorage.clear();
      navigate("/login");
    }
  };
